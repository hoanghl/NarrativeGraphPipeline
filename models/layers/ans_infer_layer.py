...
class Decoder(torch_nn.Module):
    def __init__(self, l_a, d_vocab, d_hid, tokenizer, embd_layer):
        super().__init__()
    ...
    def forward(self, Y, input_ids, input_masks):
        ...
        encoded = self.embd_layer.encode_ans(input_ids=input_ids, input_masks=input_masks)
        # [b_, l_, d_hid]
    ...
    def choose_scheduled_sampling(self, output, a_ids, cur_step, max_step):
        ...
        return a_ids if t == 0 else output
    ...
    def generate(self, decoder_input_ids, encoder_outputs):
        ...
        decoder_input_mask = torch.ones(decoder_input_ids.shape, device=encoder_outputs.device)
