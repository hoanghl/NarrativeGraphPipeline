...
class Decoder(torch_nn.Module):
    def __init__(self, la, d_bert, d_vocab, tokenizer, embd_layer, criterion):
        super().__init__()
        ...
        self.ff = torch_nn.Linear(d_bert, d_vocab, bias=False)
    ...
    def do_train(self, Y, a_ids, a_masks, cur_step, max_step):
        ...
        input_embds = torch.zeros((bz, la, self.d_bert), dtype=Y.dtype, device=Y.device)
