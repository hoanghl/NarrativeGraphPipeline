...
from utils.model_utils import Encoder


class CHIME(nn.Module):
    def __init__(self, path_pretrained):
        super(CHIME, self).__init__()

        config = AutoConfig.from_pretrained(path_pretrained, output_hidden_states=True)
        pretrained_encoder = AutoModel.from_pretrained(path_pretrained, config=config)
        tokenizer = AutoTokenizer.from_pretrained(path_pretrained)
        ...
        self.pad_i = tokenizer.pad_token_id
        self.msk_i = tokenizer.mask_token_id
        self.cls_i = tokenizer.cls_token_id
        self.sep_i = tokenizer.sep_token_id
    ...
    def dynamic_padding(self, q, r, a):
        ...
        bsz = q.size(0)
        qra = (
            torch.ones((bsz, 1 + q.size(1) + 1 + r.size(1) + 1 + a.size(1) + 1)).to(q.device)
            * self.pad_i
        )
        ...
        for bsz_i in range(bsz):
            ...
            n_c_c = r_c[r_c != self.pad_i].long()
            n_a_c = a_c[a_c != self.pad_i].long()

            # masked q + r & a -> inputs
            # q + r & shifted a -> labels
            n_qra = torch.cat(
                (self.cls_i, n_q_c, self.cls_i, n_c_c, self.sep_i, n_a_c, self.sep_i),
                dim=0,
            )
            t_qra = torch.cat((self.cls_i, n_q_c, self.cls_i, n_c_c, n_a_c, self.sep_i), dim=0)
            ...
            qAr_mask[
                bsz_i,
                len(n_qra) : (len(n_qra) + r.size(1) + q.size(1) - len(n_c_c) - len(n_q_c)),
            ] = 1
            ans_mask[bsz_i, sen_1_len - 1 : len(n_qra)] = 1  # a parts
            # filling parts for a
            ans_mask[bsz_i, (len(n_qra) + r.size(1) + q.size(1) - len(n_c_c) - len(n_q_c)) :] = 1
    ...
    def forward(self, qrah):
        ...
        for i in range(ans.size(1)):
            for j in range(rev.size(1)):
                qra, trg, ans_mask, qAr_mask, seg_mask, a_mask = self.dynamic_padding(
                    que, rev[:, j, :], ans[:, i, :]
                )
                if self.model_type == "xlnet":
                    hidden_states = self.encoder(
                        input_ids=qra, token_type_ids=seg_mask, perm_mask=(1.0 - a_mask)
                    )
                elif self.model_type == "bert":
                    hidden_states = self.encoder(
                        input_ids=qra, token_type_ids=seg_mask, attention_mask=a_mask
                    )
                last_hidden_states = hidden_states[0]
                c_memory, a_memory, cgate = self.memoryupdater(
                    c_memory, a_memory, qAr_mask, ans_mask, last_hidden_states
                )
